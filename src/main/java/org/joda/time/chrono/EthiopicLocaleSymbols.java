 ...
 * @since 1.0
 */
class EthiopicLocaleSymbols {

    private static ConcurrentMap<Locale, EthiopicLocaleSymbols> cCache = new ConcurrentHashMap<Locale, EthiopicLocaleSymbols>();
     ...
     */
    static EthiopicLocaleSymbols forLocale(Locale locale) {
        if (locale == null) {
            locale = Locale.getDefault();
        }
        EthiopicLocaleSymbols symbols = cCache.get(locale);
        if (symbols == null) {
            symbols = new EthiopicLocaleSymbols(locale);
            EthiopicLocaleSymbols oldSymbols = cCache.putIfAbsent(locale, symbols);
            if (oldSymbols != null) {
                symbols = oldSymbols;
            }
        }
    ...
    // There are 13 months in the Ethiopic calender
     ...
     */
    private EthiopicLocaleSymbols(Locale locale) {
        ...
        addNumerals(iParseMonths, 1, 13, integers);
